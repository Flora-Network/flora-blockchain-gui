...
function loadConfig(net) {
  try {
    // check if FLORA_ROOT is set. it overrides 'net'
    const config_root_dir =
      'FLORA_ROOT' in process.env
        ? process.env.FLORA_ROOT
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 55422); // jshint ignore:line
